...
### Coroutine context-> coroutine elements-> Job ,Dispatcher,Error handling,Name
- Job controls lifecycle of coroutine execution
- Dispatcher.Main,Dispatcher.Default, Dispatcher.IO,Dispatcher.unconfined
- Dispatcher.Default: For CPU intensive operations
...
### Principle 1: Every coroutine needs to be started in specific logical scope with a limited life time.
- Benefit: As compared to RX , coroutine would not even start without scope, in RX compiler wont even warn you if you don't collect the disposables.
- viewmodelscope will limit the usage till viewmodel is not discarded by android system, so CR gets destroyed once viewmodel gets cleared.

### Principle 2: CR started in same scope forms a hierarchy. 
- Through Job inside each CR , each CR forms parent child relationship. 
- Each CR scope can again launch a new CR with its own Job and context.

### Principle 3: A parent job wont complete until all its child Jobs gets completed.

### Principle 4: Cancelling a parent job would cancel all its children, but cancelling any siblings wont
cancel parent

### Principle 5: If child coroutines gets failed , exception is propagated upwards. And Depending upon the job type
either all siblings gets cancelled or not.


### Important principle 5:
- If scope is using a plain Job then if child CR gets exception , child CR + parent scope both gets cancelled.
- If scope is using a Supervisor JOb then of child CR gets exception , only Child CR gets cancelled , siblings and parent scope will not gets cancelled.
- Viewmodel scope uses a supervisor job 

### 13. Exception Handling

- exception thrown using launch scope will be propagated up and if scope had exception handler its handled inside them.
- Same thing get reverse when use async , in this case exception is not propagated up and returned in resulting Deferred object.
- When to use try catch vs exception handler
- If use case is to retry something in exception and want to recover then try-catch
- If want to cancel CR and don't want to recover then use exception handler
...
### Flow builders:
- flowOf,asFlow and flow.
- Flow is similar to streams in dart.
- Flow emits the values multiple times and every emitted item  collected inside using collect.
- Terminal operators collect,first ,last,single, toSet, toList, fold and reduce.

### Flow lifecycle operators
- onStart, onCompletion, onEach

### Terminal Operator as LiveData
- When app is in background "Fetching current stock prices" is still being printed even when app is in bg.
- What we want is to stop the fetching when app in bg and resume again using flow
- asLiveData()

### Intermediate flow operator
- map, filter(predicate), filterNot, mapNotNull, take, takeWhile(predicate)
- 
...
### Difference between Cold flow and Shared(Hot) flow
- Cold flow gets active when collect is used / Hot flow emits without collect
- Cold flow gets deactivated after exception/ reverse in hot flow
- Multiple launch can create multiple emits for same flow.
- In shared flow as name suggest: multiple emits are shared with common flow
- shareIn converts the cold flow into hot flow
- used shareIn ==> shareIn(
  scope = viewModelScope,
  started = SharingStarted.WhileSubscribed()
  )
- on config change shared flow also stops collecting so one way is to pass value inside whileSubscribed
- This waits after last collector stops collecting in milliseconds.
- Combination of:  .shareIn(
  scope = viewModelScope,
  started = SharingStarted.WhileSubscribed(
  stopTimeoutMillis = 5000
  ),
  replay = 1 //retain no of emission to restore
  )

### Diff between Sharedflow and Stateflow
                  Shared flow	   State flow
1. Initial Value	   No	          Yes(loading)
2. Replay cache	   customizable	   1
3. emission of 
subsequent equal   yes             no
values	

-- Usage rules:
- When using hot flow , always go with State flow
- Stateflows are more efficient than hot flows
- Stateflows provide more convenient option to read/write its value in non-suspending fashion by synchronously accessing value property
## Author
